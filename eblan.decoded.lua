...
--[[Удален большой блок Languages для экономии памяти]]
...
--[[local Languages = {
    ...
    Russian = {
        ...
        YBAStandRange = "YBA Дальность стенда",
        UndergroundFlight = "Подземный полет",
...
-- Экспорт функций для модуля автоселла (ПОСЛЕ объявления всех функций)
_G.startNoClip = startNoClip
_G.stopNoClip = stopNoClip
_G.startFly = startFly
_G.stopFly = stopFly
_G.MovementConfig = MovementConfig
_G.isNoClipping = function() return isNoClipping end
_G.isFlying = function() return isFlying end
...
local function createOrUpdateESP(p)
    -- Проверяем что игрок и его персонаж существуют
    ...
    -- Безопасная проверка на существование HumanoidRootPart
    local localPlayer = Players.LocalPlayer
...
local function startItemESP()
    ...
    local itemESPLoop = RunService.Heartbeat:Connect(function()
            ...
            for i = 1, math.min(100, #items) do 
                ...
                if item and item.Object and item.Object.Parent then
                    if YBAConfig.ItemESP.Items[item.Name] then
                        -- Предмет включен в настройках - показываем ESP
                        ...
                        -- Предмет отключен в настройках - убираем ESP
                        if itemESPElements[item.Object] then
                            print("YBA Item ESP: Предмет", item.Name, "отключен в настройках - удаляем ESP")
        ...
        for obj, esp in pairs(itemESPElements) do
            -- Удаляем ESP если предмет больше не найден ИЛИ отключен в настройках
            if not currentItems[obj] or (esp and esp.itemName and not YBAConfig.ItemESP.Items[esp.itemName]) then
                if esp and esp.itemName and not YBAConfig.ItemESP.Items[esp.itemName] then
                    print("YBA Item ESP: Очистка - предмет", esp.itemName, "отключен в настройках")
                end
...
-- Функция проверки, не превышено ли количество попыток
...
local function findAutofarmItems()
    ...
    -- Ищем по точному пути workspace > Item_Spawns > Items
    local itemSpawnsFolder = workspace:FindFirstChild("Item_Spawns")
    if not itemSpawnsFolder then
        print("AUTOFARM: Папка Item_Spawns не найдена!")
        return items
    end
    
    local itemsFolder = itemSpawnsFolder:FindFirstChild("Items")
    if not itemsFolder then
        print("AUTOFARM: Папка Items не найдена!")
        return items
    end
    
    print("AUTOFARM: Папки найдены, сканируем модели...")
    local foundCount = 0
    
    for _, model in ipairs(itemsFolder:GetChildren()) do
        if model:IsA("Model") then
            -- Получаем название предмета из ProximityPrompt
            ...
            if itemName == "Dio's Diary" and AutofarmConfig.Items["DIO's Diary"] then
                itemName = "DIO's Diary" -- стандартизируем название
            end
            
            -- Дополнительная проверка для стрел (разные варианты названий)
            ...
            if not shouldSkipItem and AutofarmConfig.Items[itemName] then
                ...
                if worldPivot then
                    ...
                    if not isItemMaxAttempts(itemKey) then
                        ...
                        -- Добавляем в список пропущенных для повторной попытки позже
                        autofarmSkippedItems[itemKey] = tempItem
                    end
                end
            end -- Закрываем if AutofarmConfig.Items[itemName] then
        end
    end
    ...
    -- Убираем спам про количество найденных предметов
    return items
end
...
local function moveToPosition(targetPosition, callback)
    ...
    -- НОВАЯ ЛОГИКА: Перемещаемся под землю к предмету для скрытности
    local undergroundOffset = -1.6 -- На 1.5 единицы под землей для скрытности 
    local adjustedTarget = Vector3.new(targetPosition.X, targetPosition.Y + undergroundOffset, targetPosition.Z)
    
    print("AUTOFARM: 🔥 ПОДЗЕМНЫЙ полет к позиции", targetPosition, "-> под землю:", adjustedTarget)
    
    -- Включаем NoClip для прохода через землю
    if AutofarmConfig.UseNoClipMovement and not isNoClipping then
        startNoClip()
        print("AUTOFARM: NoClip включен для подземного движения")
    end
    
    -- Настраиваем персонажа для полета (как в обычном флае)
    ...
    moveConnection = RunService.Heartbeat:Connect(function()
        ...
        if distance > 3 then
            ...
            -- Используем систему как в обычном флае для плавного движения
            local moveVector = direction * (AutofarmConfig.FlightSpeed * 10) -- как в обычном флае
            bodyVelocity.Velocity = moveVector
        else
            -- Достигли цели - дополнительная проверка правильности перемещения
            bodyVelocity.Velocity = Vector3.new(0, 0, 0)
            
            -- ПРОВЕРКА: действительно ли мы на правильной позиции?
            local finalDistance = (adjustedTarget - humanoidRootPart.Position).Magnitude
            print("AUTOFARM: Проверка позиции - расстояние до цели:", math.floor(finalDistance), "м")
            
            if finalDistance <= 5 then
                print("AUTOFARM: ✅ Успешно достигли позиции предмета")
                moveConnection:Disconnect()
                if callback then
                    callback()
                end
            else
                print("AUTOFARM: ⚠️ Неточное перемещение, корректируем позицию...")
                -- Корректируем позицию принудительно
                ...
                print("AUTOFARM: После коррекции расстояние:", math.floor(correctedDistance), "м")
...
local function pickupItem(item)
    print("AUTOFARM: Пытаемся подобрать предмет", item.Name)
    
    -- ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: предмет все еще включен в настройках автофарма
    if not AutofarmConfig.Items[item.Name] then
        print("AUTOFARM: Предмет", item.Name, "был отключен в настройках во время подбора - пропускаем")
        task.spawn(function()
    ...
    local character = player.Character
    if not character then
        print("AUTOFARM: Персонаж не найден при подборе")
    ...
    local humanoidRootPart = character:FindFirstChild("HumanoidRootPart")
    if not humanoidRootPart then
        print("AUTOFARM: HumanoidRootPart не найден при подборе")
        autofarmPickingUp = false
        task.spawn(function()
            if isAutofarmEnabled then
                processNextItem()
    ...
    -- YBA механика: приближаемся к предмету в радиус 6 метров и зажимаем E на 0.6 секунды
    ...
    pickupConnection = RunService.Heartbeat:Connect(function()
        if not isAutofarmEnabled then
            -- При отключении автофарма ОБЯЗАТЕЛЬНО отпускаем клавишу
            if isKeyPressed then
                ...
                isKeyPressed = false
                print("AUTOFARM: Клавиша E отпущена при отключении автофарма")
            end
        ...
        -- Проверяем что предмет еще физически существует
        ...
        if not itemStillExists then
            print("AUTOFARM: Предмет", item.Name, "исчез (нет физических частей) - возможно подобран")
            if isKeyPressed then
                pcall(function()
                    if game:GetService("VirtualInputManager") then
                        game:GetService("VirtualInputManager"):SendKeyEvent(false, AutofarmConfig.PickupKey, false, game)
            ...
            -- Предмет исчез - проверяем был ли это реальный подбор
            ...
            if hasStartedPickup and actualPickupTime >= (AutofarmConfig.PickupDuration * 0.2) then
                -- Предмет исчез ПОСЛЕ того как мы начали жать E - это успешный подбор
                local attempts = (autofarmItemAttempts[itemKey] or 0) + 1
                resetItemAttempts(itemKey)
                autofarmLastPickupTime = tick() -- Всегда обновляем таймер при исчезновении предмета
                print("AUTOFARM: ✅ Предмет", item.Name, "УСПЕШНО подобран за", attempts, "попытки (исчез после", math.floor(actualPickupTime * 100) / 100, "сек)")
            else
                -- Предмет исчез слишком быстро - возможно баг, не считаем успешным подбором
                incrementItemAttempts(itemKey)
                print("AUTOFARM: ⚠️ Предмет", item.Name, "исчез подозрительно быстро (", math.floor(actualPickupTime * 100) / 100, "сек) - возможно баг")
            end
            
            pickupConnection:Disconnect()
            autofarmPickingUp = false
            
            -- Быстро переходим к следующему предмету (предмет уже исчез)
            task.spawn(function()
                task.wait(0.1) -- минимальная пауза для стабилизации
                if isAutofarmEnabled then
                    processNextItem()
                end
            end)
        ...
        -- Проверяем расстояние до предмета (с учетом подземного положения)
        local distance = (humanoidRootPart.Position - item.Position).Magnitude
        
        -- ОСОБАЯ ЛОГИКА: Проверяем расстояние только по X и Z координатам для подземного подбора  
        local horizontalDistance = math.sqrt((humanoidRootPart.Position.X - item.Position.X)^2 + (humanoidRootPart.Position.Z - item.Position.Z)^2)
        
        -- Если в радиусе подбора и еще не начали подбор (используем горизонтальное расстояние)
        if horizontalDistance <= AutofarmConfig.PickupRadius and not hasStartedPickup then
            print("AUTOFARM: 🔧 Начинаем подбор", item.Name, "(", math.floor(distance), "м)")
            hasStartedPickup = true
            pickupStartTime = tick()
            
            -- Зажимаем клавишу E (эмулируем нажатие как в реальном времени)
            pcall(function()
                ...
                -- Дополнительно эмулируем через UserInputService для некоторых игр
        ...
        elseif horizontalDistance > AutofarmConfig.PickupRadius and not hasStartedPickup then
            -- Если слишком далеко, подлетаем ближе (ПОДЗЕМНЫМ способом)
            -- Убираем спам про подлет
            -- Летим к позиции предмета БЕЗ отклонения по Y (функция moveToPosition сама добавит подземный offset)
        ...
        -- Если начали подбор, проверяем прошло ли нужное время
        if hasStartedPickup and tick() - pickupStartTime >= AutofarmConfig.PickupDuration then
            ...
            if isKeyPressed then
                pcall(function()
                    ...
                    -- Дополнительно эмулируем отпускание через UserInputService
                    game:GetService("UserInputService").InputEnded:Fire(
                        {KeyCode = AutofarmConfig.PickupKey, UserInputType = Enum.UserInputType.Keyboard},
                        false
                    )
            ...
            print("AUTOFARM: Завершили попытку подбора", item.Name)
            
            -- Увеличиваем счетчик попыток для предмета
            local itemKey = getItemKey(item)
            local attempts = incrementItemAttempts(itemKey)
            print("AUTOFARM: Попытка", attempts, "/3 для предмета", item.Name)
            
            -- ФИКС: Обновляем таймер только если подбор был успешным за 1-2 попытки
            if attempts <= 2 then
                autofarmLastPickupTime = tick()
                print("AUTOFARM: ✅ Успешный подбор за", attempts, "попытки - таймер обновлен")
            else
                print("AUTOFARM: ⚠️ Подбор неуспешен -", attempts, "попытки, таймер НЕ обновлен")
            end
            
            pickupConnection:Disconnect()
            autofarmPickingUp = false
            
            -- ОПТИМИЗИРОВАНО: Ждем пока предмет исчезнет или максимум 3 секунды
            task.spawn(function()
                local waitStartTime = tick()
                local maxWaitTime = 3 -- максимум 3 секунды ожидания
                
                -- Активно проверяем исчез ли предмет
                while isAutofarmEnabled and (tick() - waitStartTime) < maxWaitTime do
                    local itemStillExists = false
                    pcall(function()
                        if item.Model and item.Model.Parent then
                            local worldPivot = item.Model:GetPivot()
                    ...
                    if not itemStillExists then
                        -- Предмет исчез - сразу переходим к следующему
                        print("AUTOFARM: ✅ Предмет исчез - сразу ищем следующий (ускорение)")
                        break
                    end
        ...
        -- Таймаут общий - если слишком долго пытаемся подобрать
        if tick() - pickupStartTime > 10 then
            print("AUTOFARM: Общий таймаут для", item.Name)
            if isKeyPressed then
                pcall(function()
                    if game:GetService("VirtualInputManager") then
                        game:GetService("VirtualInputManager"):SendKeyEvent(false, AutofarmConfig.PickupKey, false, game)
            ...
            -- Увеличиваем счетчик попыток для предмета (таймаут)
            local itemKey = getItemKey(item)
            local attempts = incrementItemAttempts(itemKey)
            print("AUTOFARM: Таймаут - попытка", attempts, "/3 для предмета", item.Name)
            
            -- ФИКС: Обновляем таймер только если это была 1-2 попытка
            if attempts <= 2 then
                autofarmLastPickupTime = tick()
                print("AUTOFARM: ✅ Таймаут на", attempts, "попытке - таймер обновлен")
            else
                print("AUTOFARM: ⚠️ Таймаут на", attempts, "попытке, таймер НЕ обновлен")
            end
...
function processNextItem()
    ...
    -- Если уже что-то подбираем, ждем
    if autofarmPickingUp then
        -- Убираем спам про ожидание подбора
        return
    end
    
    -- Проверяем неактивность (15 секунд без подбора)
    -- Таймер должен быть инициализирован (> 0) и прошло больше 15 секунд
    local currentTime = tick()
    ...
    if timeSinceLastPickup > 15 then
        print("AUTOFARM: Неактивность", math.floor(timeSinceLastPickup), "секунд - ПОЛНЫЙ ПЕРЕЗАПУСК автофарма")
        
        task.spawn(function()
            -- Полностью останавливаем автофарм
            stopAutofarm()
            
            -- Ждем немного для корректного завершения
            task.wait(1)
            
            -- Запускаем автофарм заново (как будто пользователь нажал кнопку)
            print("AUTOFARM: Автоматический перезапуск...")
            startAutofarm()
        end)
        return
    end
    
    -- ОТЛАДКА: Показываем состояние таймера каждый раз
    local currentTime = tick()
    local timeSinceLastPickup = autofarmLastPickupTime and autofarmLastPickupTime > 0 and (currentTime - autofarmLastPickupTime) or -1
    if timeSinceLastPickup > 0 then
        print("AUTOFARM DEBUG: Прошло", math.floor(timeSinceLastPickup), "секунд с последнего успешного подбора")
    else
        print("AUTOFARM DEBUG: Таймер не инициализирован")
    end
    
    -- Сканируем новые предметы
    local items = findAutofarmItems()
    if #items == 0 then
        -- Если нет доступных предметов, проверяем пропущенные
        if next(autofarmSkippedItems) then
            print("AUTOFARM: Обычные предметы закончились, проверяем пропущенные предметы")
            
            -- Фильтруем пропущенные предметы - удаляем те, что возвращались более 5 раз
            ...
            for itemKey, skippedItem in pairs(autofarmSkippedItems) do
                ...
                if returnCount <= 3 then
                    ...
                    print("AUTOFARM: Возврат к предмету", skippedItem.Name, "- попытка", returnCount, "/3")
                else
                    print("AUTOFARM: Предмет", skippedItem.Name, "забыт навсегда - превышено количество возвратов (3/3)")
                    autofarmSkippedReturns[itemKey] = nil -- Очищаем счетчик
                end
            end
            
            if next(validSkippedItems) then
                print("AUTOFARM: Возвращаемся к", table.getn and table.getn(validSkippedItems) or "нескольким", "пропущенным предметам")
                -- Сбрасываем попытки только для валидных пропущенных предметов
                for itemKey, skippedItem in pairs(validSkippedItems) do
                    resetItemAttempts(itemKey)
                end
                autofarmSkippedItems = {}
                -- Быстро перезапускаем поиск
                task.spawn(function()
                    task.wait(0.1) -- минимальная пауза для стабилизации
                    if isAutofarmEnabled then
                        processNextItem()
                ...
                return
                 ...
                 print("AUTOFARM: Все пропущенные предметы забыты - сканируем карту еще раз")
                 autofarmSkippedItems = {}
                 
                 -- Делаем еще одно сканирование карты
                 task.spawn(function()
                     task.wait(1) -- Небольшая пауза
                     if not isAutofarmEnabled then return end
                     
                     local finalScanItems = findAutofarmItems()
                     if #finalScanItems > 0 then
                         print("AUTOFARM: При повторном сканировании найдено", #finalScanItems, "новых предметов!")
                         processNextItem()
                     else
                         print("AUTOFARM: При повторном сканировании новых предметов не найдено - выключаемся на 60 секунд")
                         ...
                         autofarmRestartTimer = task.spawn(function()
                             ...
                             if autofarmAutoRestarting then
                                 print("🤖 AUTOFARM: Автоматический перезапуск через 60 секунд")
                                 autofarmAutoRestarting = false
                                 startAutofarm()
                             end
                         end)
                         
                         print("🤖 AUTOFARM: Выключен на 60 секунд - ждем появления новых предметов")
                     end
                 end)
                 return
             end
        else
            -- Убираем спам про отсутствие предметов
            -- Используем spawn чтобы избежать блокировки
            task.spawn(function()
    ...
    -- ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: предмет еще существует перед полетом
    local itemExists = false
    pcall(function()
        if nextItem.Model and nextItem.Model.Parent then
            -- Проверяем что у модели есть WorldPivot и позиция валидна
            local worldPivot = nextItem.Model:GetPivot()
            if worldPivot and worldPivot.Position and worldPivot.Position.Magnitude > 0 then
                -- Дополнительно проверяем что модель имеет физические части
                local hasPhysicalParts = false
    ...
    if not itemExists then
        print("AUTOFARM: Предмет", nextItem.Name, "исчез ДО начала полета (нет физических частей) - сразу ищем следующий")
        task.spawn(function()
            task.wait(0.1) -- минимальная пауза для стабилизации
            if isAutofarmEnabled then
                processNextItem()
    ...
    -- Летим к предмету (с отступом 1.5 единицы под землей)
    local targetPosition = Vector3.new(nextItem.Position.X, nextItem.Position.Y - 1.6, nextItem.Position.Z)
    pcall(function()
        moveToPosition(targetPosition, function()
        if isAutofarmEnabled then
            -- ПОВТОРНАЯ ПРОВЕРКА: предмет еще существует после полета
            ...
            if itemExistsAfterFlight then
                ...
                print("AUTOFARM: Предмет", nextItem.Name, "исчез ПОСЛЕ полета (нет физических частей) - сразу ищем следующий")
                task.spawn(function()
                    task.wait(0.1) -- минимальная пауза для стабилизации
                    if isAutofarmEnabled then
                        processNextItem()
...
-- Функция принудительной прогрузки всей карты
local function forceLoadEntireMap()
    print("🤖 AUTOFARM: Начинаем принудительную прогрузку карты...")
    ...
    -- Включаем NoClip для свободного движения
    if not isNoClipping then
        pcall(function()
            startNoClip()
        end)
    ...
    -- Оптимизированные точки для прогрузки карты (7 точек: углы + центр + Diavolo)
    local loadPoints = {
        -- Центр карты
        Vector3.new(0, 70, 0),
        
        -- 4 угла карты (увеличенное расстояние)
        Vector3.new(600, 70, 600),   -- Северо-Восток
        Vector3.new(-600, 70, 600),  -- Северо-Запад
        Vector3.new(600, 70, -600),  -- Юго-Восток
        Vector3.new(-600, 70, -600), -- Юго-Запад
        
        -- Высотная точка для вертикального покрытия
        Vector3.new(0, 150, 0),      -- Центр высоко
        
        -- Область босса Diavolo (Boss Arena) - добавлено для лучшего покрытия
        Vector3.new(900, 70, 45), -- Позиция левее Diavolo для прогрузки boss arena
    }
    
    print("🤖 AUTOFARM: Летим по", #loadPoints, "точкам для прогрузки карты...")
    
    -- Создаем BodyVelocity для полета (как в автофарме)
    local bodyVelocity = Instance.new("BodyVelocity", humanoidRootPart)
    bodyVelocity.MaxForce = Vector3.new(9e9, 9e9, 9e9)
    bodyVelocity.Velocity = Vector3.new(0, 0, 0)
    
    for i, point in ipairs(loadPoints) do
        if not isAutofarmEnabled then break end -- Прерываем если автофарм отключили
        
        -- Летим к точке прогрузки (используем полет как в автофарме)
        print("🤖 AUTOFARM: Летим к точке прогрузки", i, "/", #loadPoints, "- позиция:", point)
        
        local startTime = tick()
        local maxFlightTime = 10 -- Максимум 10 секунд на одну точку
        ...
        while isAutofarmEnabled and tick() - startTime < maxFlightTime do
            ...
            if distance > 5 then
                ...
                -- Используем ту же скорость что и в автофарме
                local moveVector = direction * (AutofarmConfig.FlightSpeed * 10)
                bodyVelocity.Velocity = moveVector
            else
                -- Достигли точки прогрузки
                bodyVelocity.Velocity = Vector3.new(0, 0, 0)
                break
            end
            
            task.wait(0.03) -- Плавное обновление полета
        end
        
        -- Поворачиваем камеру для лучшей прогрузки
        camera.CFrame = CFrame.new(point + Vector3.new(0, 50, 0), point)
        
        -- МАКСИМАЛЬНО БЫСТРАЯ прогрузка но с гарантией
        task.wait(0.1)
        
        -- Показываем прогресс каждой точки
        print("🤖 AUTOFARM: Прогрузка точки", i, "/", #loadPoints, "- позиция:", point, "завершена")
    end
    
    -- Очищаем BodyVelocity после прогрузки
    if bodyVelocity and bodyVelocity.Parent then
        bodyVelocity:Destroy()
    end
    
    -- Возвращаемся в исходную позицию через полет
    print("🤖 AUTOFARM: Возвращаемся в исходную позицию через полет")
    ...
    local maxReturnTime = 15 -- Максимум 15 секунд на возврат
    ...
    -- Быстрая финальная стабилизация
    task.wait(1)
    
    print("🤖 AUTOFARM: Прогрузка карты завершена! Загружено", #loadPoints, "точек (включая область Diavolo). Начинаем поиск предметов...")
end
...
function startAutofarm()
    ...
    -- Отменяем автоматический перезапуск если он был запланирован
    if autofarmAutoRestarting then
        autofarmAutoRestarting = false
        ...
        print("🤖 AUTOFARM: Отменен автоматический перезапуск - запуск вручную")
    end
    ...
    -- Настраиваем отслеживание смерти игрока для автофарма
    local function setupAutofarmDeathTracking()
        local humanoid = character:FindFirstChild("Humanoid")
        if humanoid and not autofarmDeathCheckConnection then
            autofarmDeathCheckConnection = humanoid.Died:Connect(function()
                print("🤖 AUTOFARM: Игрок умер! Сохраняем состояние функций и останавливаем...")
                
                -- Сохраняем состояние функций перед смертью
                wasAutofarmEnabledBeforeDeath = AutofarmConfig.Enabled
                wasAutosellEnabledBeforeDeath = _G.AutosellModule and _G.AutosellModule.isEnabled() or false
                deathTrackingActive = true
                
                print("🤖 AUTOFARM: Состояние сохранено - Autofarm:", wasAutofarmEnabledBeforeDeath, "Autosell:", wasAutosellEnabledBeforeDeath)
                
                -- Останавливаем функции
                stopAutofarm()
                if _G.AutosellModule and _G.AutosellModule.stop then
                    print("🤖 AUTOFARM: Останавливаем автосел из-за смерти игрока...")
    ...
    -- Устанавливаем отслеживание смерти
    setupAutofarmDeathTracking()
    
    -- Переустанавливаем отслеживание при респавне И восстанавливаем функции
    Players.LocalPlayer.CharacterAdded:Connect(function()
        task.wait(1) -- Ждем загрузки персонажа
        
        -- Восстанавливаем функции если они были включены перед смертью
        if deathTrackingActive then
            print("🤖 AUTOFARM: Игрок возродился! Восстанавливаем функции...")
            print("🤖 AUTOFARM: Восстанавливаем состояние - Autofarm:", wasAutofarmEnabledBeforeDeath, "Autosell:", wasAutosellEnabledBeforeDeath)
            
            -- Восстанавливаем автофарм если он был включен
            if wasAutofarmEnabledBeforeDeath then
                task.wait(2) -- Даем время персонажу полностью загрузиться
                print("🤖 AUTOFARM: Перезапускаем автофарм после возрождения...")
                if not isAutofarmEnabled then
                    startAutofarm()
                end
            end
            
            -- Восстанавливаем автосел если он был включен  
            if wasAutosellEnabledBeforeDeath then
                task.wait(3) -- Даем время автофарму запуститься сначала
                print("🤖 AUTOFARM: Перезапускаем автосел после возрождения...")
                if _G.AutosellModule and _G.AutosellModule.start and not _G.AutosellModule.isEnabled() then
                    _G.AutosellModule.start()
                end
            end
            
            -- Сбрасываем флаги после восстановления
            deathTrackingActive = false
            wasAutofarmEnabledBeforeDeath = false
    ...
    -- Очищаем предыдущие данные о попытках и пропущенных предметах
    autofarmItemAttempts = {}
    autofarmSkippedItems = {}
    autofarmSkippedReturns = {}
    
    -- Не сохраняем исходную позицию - остаемся где остановились (как телепорт)
    
    -- НОВОЕ: Принудительная прогрузка карты перед началом фарма
    task.spawn(function()
        pcall(function()
            forceLoadEntireMap()
        end)
        
        -- ВАЖНО: Инициализируем время последнего подбора ПОСЛЕ прогрузки карты
        autofarmLastPickupTime = tick()
        print("🤖 AUTOFARM: Таймер неактивности запущен после прогрузки карты")
        
        -- После прогрузки начинаем основной процесс
        if isAutofarmEnabled then
            pcall(function()
                processNextItem()
            end)
...
function stopAutofarm()
    ...
    if autofarmAutoRestarting then
        ...
        print("🤖 AUTOFARM: Отменен автоматический перезапуск - остановка вручную")
    end
    
    -- ПРИНУДИТЕЛЬНО отпускаем клавишу E (фикс бага с зажатой кнопкой)
    pcall(function()
        if game:GetService("VirtualInputManager") then
            game:GetService("VirtualInputManager"):SendKeyEvent(false, AutofarmConfig.PickupKey, false, game)
        end
        ...
        print("🤖 AUTOFARM: Клавиша E принудительно отпущена")
    end)
    
    -- Отключаем отслеживание смерти для автофарма
    if autofarmDeathCheckConnection then
        autofarmDeathCheckConnection:Disconnect()
        autofarmDeathCheckConnection = nil
    end
    ...
    if character then
        ...
        if humanoidRootPart then
            ...
            -- Восстанавливаем настройки персонажа ТОЛЬКО если флай НЕ включен
        ...
        -- Отключаем NoClip если включали
        if AutofarmConfig.UseNoClipMovement and isNoClipping then
            stopNoClip()
        end
    end
    
    print("AUTOFARM: Полностью остановлен")
end

-- Экспорт функций автофарма для модуля автоселла  
_G.moveToPosition = moveToPosition
_G.isAutofarmEnabled = function() return isAutofarmEnabled end
_G.startAutofarm = startAutofarm
_G.stopAutofarm = function() 
    isAutofarmEnabled = false 
    print("🤖 AUTOFARM: Остановлен через _G.stopAutofarm") 
end
_G.AutofarmConfig = AutofarmConfig
